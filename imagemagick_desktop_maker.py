...
def create_temps(arguments: tuple[TempInput, str, str]) -> tuple[Image.Image, TempImages, str, str]:
    ...
    cairosvg.svg2png(
        url=os.path.join(SVGDIR, inputs.svg), write_to=tmpmask, output_height=inputs.wal.height, output_width=inputs.wal.width, scale=1
    )
...
def main():
    ...
    with Pool(int(os.cpu_count() / 2)) as pool:
        for result in tqdm(
            pool.imap_unordered(create_temps, templist),
            total=len(templist),
            desc="Creating temporaries",
            unit="image",
            ascii=True,
            dynamic_ncols=True,
        ):
